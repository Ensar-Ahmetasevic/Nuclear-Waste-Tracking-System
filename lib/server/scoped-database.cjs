...
// Prisma 7 DMMF no longer includes relationFromFields / isRequired / hasDefaultValue.
// Required/defaulted scalars come from generated schema-field-meta.cjs (see db:generate).
// Foreign keys are recovered as conventional relationName + "Id" fields.
const fieldMeta = require('./schema-field-meta.cjs');
const requiredScalars = new Map(
  Object.entries(fieldMeta.requiredScalars).map(([name, fields]) => [name, new Set(fields)]),
);
const defaultedScalars = new Map(
  Object.entries(fieldMeta.defaultedScalars).map(([name, fields]) => [name, new Set(fields)]),
);

function relationForeignKey(model, relation) {
  if (relation.relationFromFields?.length) return relation.relationFromFields[0];
  const candidate = `${relation.name}Id`;
  return model.fields.some(field => field.kind === 'scalar' && field.name === candidate) ? candidate : null;
}
...
async function validateData(tx, model, data, organizationId, creating) {
  ...
  const required = requiredScalars.get(model.name) || new Set();
  const defaults = defaultedScalars.get(model.name) || new Set();
  const result = {};
  for (const [key, value] of Object.entries(data)) {
    ...
    if (value === null) {
      if (required.has(key)) throw new HttpError(400, `${key} is required`);
  ...
  if (creating) {
    for (const name of required) {
      if (name === 'organizationId' || defaults.has(name)) continue;
      if (result[name] == null) throw new HttpError(400, `${name} is required`);
    }
  }
  for (const relation of model.fields.filter(field => field.kind === 'object')) {
    const foreignKey = relationForeignKey(model, relation);
    if (!foreignKey || foreignKey === 'organizationId' || result[foreignKey] == null) continue;
    const delegate = relation.type[0].toLowerCase() + relation.type.slice(1);
    if (!tx[delegate]?.findFirst) continue;
